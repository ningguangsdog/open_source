...
def select_native_targets(
    ...
    *,
    ...
    for library in library_records:
        ...
        if not candidates:
            library_score, capabilities, reasons = _score_text(str(lib_path))
            if library_score > 0:
                targets.append(
                    {
        ...
        for kind, name in candidates:
            ...
            score, capabilities, reasons = _score_text(f"{lib_path} {name}")
            if kind == "jni_symbol":
                score += 10
...
def available_decompiler() -> str | None:
...
def run_targeted_decompile(
    ...
    *,
    ...
    return {
        ...
        "libraries_attempted": dict(grouped),
        "results": results,
    }
